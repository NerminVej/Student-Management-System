...
public class Main {  
    public static void main(String[] args) {  
        ...
        // does not get interrupted.        boolean isRunning = true;  
        while (isRunning) {  
            ...
            // This can be further expanded upon.            switch (choice) {  
                case 1:  
    ...
    // We make multiple input validation checks to see if the student is inside the course and a legitimate student.    private static void assignGrade(Scanner scanner, StudentManager studentManager, CourseManager courseManager) {  
...
My "Main" class contains the whole CLI section of the project.

Let us break down the CLI of the project one by one.
...
We create a scanner that lets us gain user input. And we create one object of the "studentManager" and the "courseManager" class that we can use to handle the students and courses.
...
We set up a while loop that runs forever until the user inputs a valid command.
Inside of the variable "choice" we store the user input which we use in a switch case statement.
...
Inside of the switch case statement we handle every case that we want to handle. We can also expand upon it in the future if we want to which is why a switch case is the go to method here.
